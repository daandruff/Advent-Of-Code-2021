(() => {
    ...
    function formatInput(input) {
        ...
        for (let i = 0; i < formated.length; i++) {
            for (let j = 0; j < formated[i].init.length; j++) {
                formated[i].init[j] = [...formated[i].init[j]].sort().join('');
            }
        }
        for (let i = 0; i < formated.length; i++) {
            for (let j = 0; j < formated[i].output.length; j++) {
                formated[i].output[j] = [...formated[i].output[j]].sort().join('');
            }
        }
    ...
    // Just count the knowns, we don't care about making this smart
    function countKnown(input) {
        let count = 0;

        for (let i = 0; i < input.length; i++) {
            for (let n = 0; n < input[i].output.length; n++) {
                switch (input[i].output[n].length) {
                    case 2:   
                    case 3:
                    case 4:
                    case 7:
    ...
    // Solve known
    function solveKnown(input) {
        for (let i = 0; i < input.length; i++) {
            for (let n = 0; n < input[i].init.length; n++) {
                switch (input[i].init[n].length) {
                    case 2:
                        input[i].key[1] = input[i].init[n];
                        break;
                    case 3:
                        input[i].key[7] = input[i].init[n];
                        break;
                    case 4:
                        input[i].key[4] = input[i].init[n];
                        break;
                    case 7:
                        input[i].key[8] = input[i].init[n];
                        break;
                }
            }
        }
    ...
    // Return list with digits containing specific num of switches on
    function getDigitsWith(init, numSegments, key) {
        let list = [];
        init.forEach((n) => {
            if (!key.includes(n)) {
                n.length === numSegments ? list.push(n) : false
            }
        });
    ...
    // Solve the unknown based on key
    function solveUnknown(input) {
        for (let i = 0; i < input.length; i++) {
            // Solve for 9
            let possible9 = getDigitsWith(input[i].init, 6, input[i].key);
            possible9.forEach(x => {
                if ([...input[i].key[4]].every(v => x.includes(v))) {
                    input[i].key[9] = x;
                }
            });
            // Solve for 3
            let possible3 = getDigitsWith(input[i].init, 5, input[i].key);
            possible3.forEach(x => {
                if ([...input[i].key[7]].every(v => x.includes(v))) {
                    input[i].key[3] = x;
                }
            });
            // Solve for 0
            let possible0 = getDigitsWith(input[i].init, 6, input[i].key);
            possible0.forEach(x => {
                if ([...input[i].key[7]].every(v => x.includes(v))) {
                    input[i].key[0] = x;
                }
            ...
            // Solve for 6
            input[i].key[6] = getDigitsWith(input[i].init, 6, input[i].key)[0];
            // Solve for 5 and 2
            let possible5 = getDigitsWith(input[i].init, 5, input[i].key);
            if ([...possible5[0]].every(v => input[i].key[9].includes(v))) {
                input[i].key[5] = possible5[0];
                input[i].key[2] = possible5[1];
            } else {
                input[i].key[5] = possible5[1];
                input[i].key[2] = possible5[0];
            }
        }
    ...
    // Convert output to decoded output
    function decodeOutput(input) {
        for (let i = 0; i < input.length; i++) {
            let decoded = '';

            input[i].output.forEach(v => {
                decoded += input[i].key.indexOf(v);
            });

            input[i].decoded = parseInt(decoded);
        }

        return input;
    }

    // Sum all decoded outputs
    ...
    function main(input) {
        ...
        console.log("The number of known numbers based on count of activated wires are: " + countKnown(data));

        data = solveKnown(data);
        data = solveUnknown(data);
        data = decodeOutput(data);
        console.log("Summed value of all decoded outputs: " + sumOutput(data));
    }
