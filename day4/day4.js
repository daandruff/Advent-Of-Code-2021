...
function constructBoard(string) {
    ...
    for (let i = 0; i < separatedLine.length; i++) {
        line.push(parseInt(separatedLine[i]));
        if (line.length == 5) {
            board.push(line);
            line = [];
        }
...
// Cross our number and then check for winner, returning index of winning board
function callNumber(number, boardlist) {
    let winner = -1;

    // Find and cross out every instance of $number
    for (let i = 0; i < boardlist.length; i++) {
        if (markedWinners.indexOf(i) === -1) {
            for (let l = 0; l < boardlist[i].length; l++) {
                let foundAt = boardlist[i][l].indexOf(number);
                if (foundAt >= 0) {
                    boardlist[i][l][foundAt] = -1;
                }
            }
        }
    }

    // Check if any boards has line on board
    for (let i = 0; i < boardlist.length; i++) {
        if (markedWinners.indexOf(i) === -1) {
...
// Return if a line is found (unspecified)
function unspecLineOnBoard(board) {
    let lineFound = false;
...
function sumRemainingBoardNumbers(board) {
    let sum = 0;

    for (let l = 0; l < board.length; l++) {
        for (let r = 0; r < board[l].length; r++) {
            sum += (board[l][r] > 0) ? board[l][r] : 0;
        }
    }
...
function initCalculation(getFirst = true) {
    let boardlist = [];
    let latestCalculation = -1;

    // Reset list of winners
    markedWinners = [];

    // Construct all boards
    ...
    for (let i = 0; i < input.length; i++) {
        let winner = callNumber(input[i], boardlist);

        if (winner >= 0) {
            latestCalculation = (input[i] * sumRemainingBoardNumbers(boardlist[winner]));
    ...
    // Return calculation based on last reported winning board
    return latestCalculation;
}

document.body.innerHTML += ('<div>The multiplied value of last draw and sum of winning board-remainder is (First winning board): ' + initCalculation() + '</div>');
document.body.innerHTML += ('<div>The multiplied value of last draw and sum of winning board-remainder is (Last winning board): ' + initCalculation(false) + '</div>');
